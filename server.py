...
from model_service2 import ModelService
...
class RecognizeAudioServicer(RecognizeServicer):
    ...
    def recognize_srt(self, request, context):
        file_name = self.write_wave_to_file(request.filename, request.audio)
        response = self.inference.get_srt(file_name, request.language, False, False)
